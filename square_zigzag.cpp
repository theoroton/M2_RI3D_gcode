...
int main () {
    ...
    //Calcul des points du carré
    p1.x = X_start+tau;     p1.y = Y_start-tau;
    p4.x =  X_start + L-tau;     p4.y = Y_start + L+tau;

    Point curr = {p1.x, p1.y};
    //Déplacement au premier point interieur
    gcode_file << "G0 X" << curr.x << " Y" << curr.y << " Z" << Z << " F" << F_G0 << ";" << "\n"; 
    float L_interieur = L;
    int counter = 0;
    int iter_max = L_interieur/tau;
    E = 0;
    while(counter != iter_max){
    //while(curr.x != p4.x && curr.y != p4.y) {
        if(counter == 0) {
            curr.y +=L_interieur - tau;
        } else {
            curr.y = (counter%2 == 0) ? curr.y+L_interieur - 2*tau : curr.y-L_interieur + 2*tau;
        }
        //Augmente E par deltaE
        E += deltaE;
        //Déplacement G1 à ce point
